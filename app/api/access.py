...
# NOVO: Koristimo pydantic settings umesto os.getenv
...
async def get_qr_access_token(
    ...
    Frontend automatically calls this to display the QR code.
    """
    now = datetime.now(timezone.utc)

    # FRONTEND UX: Block generation if there is no active subscription
...
async def scan_qr(scan_data: ScanRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        payload = jwt.decode(scan_data.qr_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
    ...
    # --- 🛡️ DATABASE VALIDATION BEFORE INSERT ---
